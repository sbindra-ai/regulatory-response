%MACRO  m_f_cum_chg (   param1 = paramcd ,
                      param2= parcat1,
                      ads = ,
                      pop = ,
                      xtick =
...
/ DES = '###Cumulative percent of subjects by percent change from baseline ###';
/*******************************************************************************
 * Bayer AG
 * Macro rely on: ###choose: TOSCA, initstudy, GMS, GDD, pure SAS###
 *******************************************************************************
 * Purpose          : Produce Cumulative percent of subjects by percent change from baseline  for the Analysis
 * Programming Spec :
...
if  int(pchg) ^ = (pchg) then pchgc = put(pchg , 12.2);
else  pchgc = put(pchg , 12.);;
drop pchg;
run;

data  &ads._1;
    set &ads._1 ;
    pchg = Input(strip(pchgc) , best. ) ;
    attrib pchg label  = "Relative change (%) from baseline" ;
RUN;
