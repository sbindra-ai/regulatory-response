 ...
 * Comments         :
 *******************************************************************************
 * Author(s)        : sgtja (Katrin Roth) / date: 03JUL2023
 * Reference prog   :
 *******************************************************************************
 * Change History   :
 ******************************************************************************/
/* Changed by       : emvsx (Phani Tata) / date: 09AUG2023
 * Reason           : Updated and Placebo+0
 ******************************************************************************/
/* Changed by       : sgtja (Katrin Roth) / date: 04SEP2023
 * Reason           : ###setting negative imputed values to 0
 ******************************************************************************/
/* Changed by       : sgtja (Katrin Roth) / date: 11SEP2023
 * Reason           : changing label for variable "scenario"
 ******************************************************************************/
 ...
 ******************************************************************************/
    ...
    *%spro_check_param(name=param2, type=TEXT)
    ...
    %m_shift(datain=step3, delta=&delta)
    ...
    PROC TRANSPOSE DATA=tp_&d OUT=tp_trans_&d /*prefix=chg*/;
        ...
        VAR Week1a Week4a Week8a Week12a;
    RUN;

    DATA step_4;
        SET tp_trans_&d;
        IF _name_='Week1a' THEN avisitn=10;
        ELSE IF _name_='Week4a' THEN avisitn=40;
    ...
    OPTIONS MPRINT;
    %m_mmrm(step_4,&byvar,&classvar,chg,&xvar,un,lsmeans,estim,pred);quit;
 ...
 *******************************************************************************/
    ...
    * Divide two-sided p-Value by 2 to get one-sided p-value ;
    DATA estim_tot_&d;
        ...
        scenario = cat("Elinzanetant 120mg+",&delta," and Placebo+0"); *delta needs to be the value of the respective (macro) variable;
