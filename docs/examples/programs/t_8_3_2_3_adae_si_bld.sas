 ...
 *******************************************************************************
 *Name of program**************************************************************/
   %iniprog(name = t_8_3_2_3_adae_si_bld);
/*
 * Purpose          : Treatment-emergent adverse events of special interest (post-menopausal uterine bleeding):
 ...
 * Reference prog   : /var/swan/root/bhc/3427080/21652/stat/main01/dev/analysis/pgms/t_8_3_2_3_adae_si_bld.sas (enpjp (Prashant Patel) / date: 09SEP2023)
 ******************************************************************************/

*get data and select only Safety patients;
%load_ads_dat(adae_view, adsDomain = adae, where = trtemfl eq "Y" and assiny = "Y" and cq04cd eq 4, adslWhere = &saf_cond.)/*TEAE of special interest cq04cd (Post-menopausal uterine bleeding): */
%load_ads_dat(adsl_view, adsDomain = adsl, where = &saf_cond, adslVars  =);
%load_ads_dat(admh_view, adsDomain = admh, adslWhere = &saf_cond, adslVars  = );
...
data adae_view2;
    merge adae_view1(in=a) mh1(in=b);
    by usubjid;
    if a and not b;
RUN;
/*N = number of subjects with uterus.*/
data adsl_view2;
    merge adsl_view1(in=a) mh1(in=b);
    by usubjid;
    if a and not b;
RUN;
