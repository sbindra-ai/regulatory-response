 ...
 * Reference prog   : /var/swan/root/bhc/3427080/21652/stat/main01/dev/analysis/pgms/t_8_3_5_8_adlb_liv_hy.sas (emvsx (Phani Tata) / date: 04OCT2023)
 ******************************************************************************/
...
data adlb_ext;
  ...
  where    &saf_cond  and  paramcd in ("SGOTSP","SGPTSP" , "BILITOSP")
            and (anl01fl = "Y" or avisitn in (500000,900000)) and  ABLFL NE "Y"  and ADY GT 1  and aval ne .;
...
data adlb_ext;
    merge adlb_ext alt_max bil_max;
    by usubjid;
if paramcd_n = "ALT_AST"  then do;
    if  paramcd in ("SGOTSP" )  and alt_max >= 3 then Crit_SGOTSP_F = 1 ;
    if  paramcd in ("SGPTSP" )  and alt_max >= 3 then Crit_SGPTSP_F = 1 ;
end ;

if paramcd_n = "BILITOSP" then do ;
    if  paramcd in ("BILITOSP" )  and bil_max >= 2 then Crit_BILITOSP_F = 1 ;
end;
...
if paramcd_n = "ALT_AST"  then do;
    ...
    if  paramcd in ("SGOTSP" )  and alt_max < 3 then Crit_SGOTSP_LU = 1 ;
    if  paramcd in ("SGPTSP" )  and alt_max   < 3 then Crit_SGPTSP_LU = 1 ;
end ;

if paramcd_n = "BILITOSP" then do ;
    B_val = 2 * anrhi  ; C_B = 1 ;
    if  paramcd in ("BILITOSP" )  and bil_max >= 2 then Crit_BILITOSP_LU = 1 ;
end;
...
if paramcd_n = "ALT_AST"  then do;
    alt_val = 3 * anrhi  ;
    if  paramcd in ("SGOTSP" )  and alt_max >= 3 then Crit_SGOTSP_rU = 1 ;
    if  paramcd in ("SGPTSP" )  and alt_max   >= 3 then Crit_SGPTSP_rU = 1 ;
end ;
if paramcd_n = "BILITOSP" then do ;
    B_val = 2 * anrhi  ;
    if  paramcd in ("BILITOSP" )  and bil_max <  2 then Crit_BILITOSP_rU = 1 ;
end;
