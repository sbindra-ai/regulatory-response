 ...
 *******************************************************************************
 *Name of program**************************************************************/
   %iniprog(name = intext_menqol);
/*
 * Purpose          : Generate intext table for mean change in MENQOL total score from baseline to Week 12 (FAS)
 * Programming Spec : 
 ...
 * Reference prog   : /var/swan/root/bhc/3427080/21652/stat/main01/dev/analysis/pgms/intext_menqol.sas (ggqct (Jose Diaz) / date: 08NOV2023)
 ******************************************************************************/

data elmean;
    set tlfmeta.t_8_2_1_4_1_adqs_ms_s_1;
...
proc sql;
alter table work.data3
  modify COL1 char(100) format=$100.
  modify comment char(100) format=$100.;
quit;
run;
...
%let enum_src = %intext_get_enumeration(meta_file = tlfmeta.t_8_2_1_4_1_adqs_ms_s_1);


title1 "Table: Mean change in MENQOL total score from baseline to Week 12 &fas_label";

footnote1 "Source: &enum_src., Table 8.2.1.11 / 2 ";
