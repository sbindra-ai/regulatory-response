 ...
 *******************************************************************************
 *Name of program**************************************************************/
   %iniprog(name = t_8_1_6_adcm_sbj_prior_cm);
/*
 * Purpose          : Prior medication: number of subjects (safety analysis set)
 * Programming Spec :
 ...
 * Reference prog   : /var/swan/root/bhc/3427080/21652/stat/main01/dev/analysis/pgms/t_8_1_6_adcm_sbj_prior_cm.sas (enpjp (Prashant Patel) / date: 21JUN2023)
 ******************************************************************************/
** >>> START: Create table - Prior medication: number of subjects <<population>>;
...
%load_ads_dat(
    adcm_view
  , adsDomain = adcm
  , where     = CMCAT = 'PRIOR AND CONCOMITANT MEDICATION' and PREFL = 'Y' /*PREFL > Pre-Treatment Flag*/
  , adslWhere = &&&pop._cond.
  , view      = N
);
