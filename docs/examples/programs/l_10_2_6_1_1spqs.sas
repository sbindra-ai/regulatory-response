 ...
 * Reference prog   : /var/swan/root/bhc/3427080/21652/stat/main01/val/analysis/pgms/l_10_2_6_1_1spqs.sas (egavb (Reema S Pawar) / date: 13JUN2023)
 ******************************************************************************/
...
PROC SORT DATA = qs_all ; BY &subj_var.  VISITNUM newdate;;RUN;
...
PROC SORT DATA = qs_final OUT = final ;
        BY &treat_var_listings_part. &subj_var.  newdate VISIT ;
        WHERE QSCAT = "PROMIS SLEEP DISTURBANCE SHORT FORM 8B V1.0";
RUN;

%LET label ='PROMIS SLEEP DISTURBANCE';
