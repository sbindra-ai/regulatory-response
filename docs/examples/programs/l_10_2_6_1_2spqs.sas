 ...
 * Reference prog   : /var/swan/root/bhc/3427080/21652/stat/main01/val/analysis/pgms/l_10_2_6_1_2spqs.sas (egavb (Reema S Pawar) / date: 13JUN2023)
 ******************************************************************************/
...
DATA QS;
    SET sp.qs;
     ...
     WHERE QSSCAT= "SEVERITY OF SLEEP DIFFICULTIES";
...
PROC SORT DATA = qs_final OUT = final NODUP;
        BY &treat_var_listings_part. &subj_var. VISIT newdate QSDTL QSTEST QSORRES;
        WHERE QSCAT = "ISI"  AND QSSCAT= "SEVERITY OF SLEEP DIFFICULTIES" ;
RUN;

%LET label ='Insomnia Severity Index (ISI)';
